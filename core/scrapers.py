...
def scrape_keejob():
    ...
    for job_element in job_cards:
        ...
        company = job_element.find("div", class_="meta_a").find("a").text.strip() if job_element.find("div", class_="meta_a") else None
        location = job_element.find("i", class_="fa fa-map-marker").next_sibling.strip() if job_element.find("i", class_="fa fa-map-marker") else None

        # Extract source logo and construct full URL
        logo_tag = job_element.find("figure").find("img") if job_element.find("figure") else None
        source_logo = url + logo_tag["src"] if logo_tag and logo_tag.has_attr("src") else None
...
def scrape_tunisie_travail():
    ...
    for job in job_cards:
        ...
        description_tag = job.find("div", style="line-height:18px;font-size:12px; font-family:Verdana, Geneva, sans-serif")
        title = title_tag.text.strip() if title_tag else "No title"
        description = description_tag.text.strip() if description_tag else "No description"
        date  = date_tag.text.strip() if date_tag else None
        job_url = title_tag['href'] 
        company = title.split(" recrute ")[0] if " recrute " in title else "No company"
